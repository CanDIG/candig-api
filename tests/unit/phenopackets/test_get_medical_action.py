...
def make_ontology_map():
    ...
    return {
        4281521:  OntologyClass(id="SNOMED:66398006",    label="Excision of breast with excision of regional lymph nodes"),
        42426830: OntologyClass(id="RxNorm:42426830",    label="Tamoxifen"),
        607996:   OntologyClass(id="SNOMED:1156506007",  label="External beam radiation therapy using photons"),
        36717353: OntologyClass(id="SNOMED:722738000",   label="Structure of bone of left femur"),
        40491905: OntologyClass(id="SNOMED:447295008",   label="Forensic intent"),
        36310520: OntologyClass(id="LOINC:LA4566-1",     label="No Evidence of this Cancer"),
        45590880: OntologyClass(id="ICD10:C23",          label="Malignant neoplasm of gallbladder"),
    }
...
def patch_all(
    ...
    *,
    ...
    mock_response_to_treatments.return_value = response_to_treatments if response_to_treatments is not None else {
        1: OntologyClass(id="LOINC:LA4566-1", label="No Evidence of this Cancer")
    }
    mock_treatment_intents.return_value = treatment_intents if treatment_intents is not None else {
        1: OntologyClass(id="SNOMED:447295008", label="Forensic intent")
    }
    mock_treatment_targets.return_value = treatment_targets if treatment_targets is not None else [
        OntologyClass(id="ICD10:C23", label="Malignant neoplasm of gallbladder")
    ]
    mock_treatment_agents.return_value = treatment_agents if treatment_agents is not None else [
        Treatment(
            agent=OntologyClass(id="RxNorm:42426830", label="Tamoxifen"),
            drug_type="PRESCRIPTION",
        )
    ]
    mock_procedures.return_value = procedures if procedures is not None else [
        Procedure(
            code=OntologyClass(id="SNOMED:66398006", label="Excision of breast with excision of regional lymph nodes"),
        )
    ]
    mock_radiation_therapies.return_value = radiation_therapies if radiation_therapies is not None else []
...
@patch("src.api.phenopacket_operations.get_medical_action_by_field", new_callable=AsyncMock)
async def test_treatment_agent_produces_medical_action(
    mock_by_field, mock_targets, mock_agents, mock_procedures,
    mock_radiation, mock_ontologies, mock_db,
):
...
async def test_radiation_therapy_produces_medical_action(
    ...
    intent    = OntologyClass(id="SNOMED:447295008",     label="Forensic intent")
    target    = OntologyClass(id="ICD10:C23",            label="Malignant neoplasm of gallbladder")
    modality  = OntologyClass(id="SNOMED:1156506007",    label="External beam radiation therapy using photons")
    body_site = OntologyClass(id="SNOMED:722738000",     label="Structure of bone of left femur")

    mock_by_field.side_effect  = [{1: response}, {1: intent}]
    mock_targets.return_value  = [target]
    mock_agents.return_value   = []
    mock_procedures.return_value   = []
    mock_radiation.return_value    = [RadiationTherapy(modality=modality, body_site=body_site, dosage=60, fractions=25)]
    mock_ontologies.return_value   = {}
    mock_db.return_value = make_mock_session([])
...
async def test_multiple_action_types_produces_correct_count(
    ...
    mock_by_field.side_effect  = [{1: response}, {1: intent}]
    mock_targets.return_value  = [target]
    mock_agents.return_value   = [Treatment(agent=OntologyClass(id="RxNorm:42426830", label="Tamoxifen"), drug_type="PRESCRIPTION")]
    mock_procedures.return_value   = [Procedure(code=OntologyClass(id="SNOMED:66398006", label="Surgery"))]
    mock_radiation.return_value    = [RadiationTherapy(
        modality=OntologyClass(id="SNOMED:1156506007", label="External beam"),
        body_site=OntologyClass(id="SNOMED:722738000",  label="Femur"),
        dosage=60, fractions=25,
    )]
    mock_ontologies.return_value = {}
    mock_db.return_value = make_mock_session([])
...
async def test_two_episodes_two_agents_produces_four_actions(
    ...
    mock_targets.return_value  = [OntologyClass(id="ICD10:C23", label="Malignant neoplasm of gallbladder")]
    mock_agents.return_value   = [
        Treatment(agent=OntologyClass(id="RxNorm:42426830", label="Tamoxifen"),    drug_type="PRESCRIPTION"),
        Treatment(agent=OntologyClass(id="RxNorm:99999999", label="Ipilimumab"),   drug_type="PRESCRIPTION"),
    ]
    mock_procedures.return_value   = []
    mock_radiation.return_value    = []
    mock_ontologies.return_value   = {}

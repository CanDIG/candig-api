            ...
            try:
                ...
                for row in result:
                    person_dict = {
                        ...
                        "birth_datetime": row.birth_datetime.isoformat() if row.birth_datetime else None,
                        "race_concept_id": row.race_concept_id,
                        "ethnicity_concept_id": row.ethnicity_concept_id,
                        "location_id": row.location_id if row.location_id is not None else 0,
                        "provider_id": row.provider_id if row.provider_id is not None else 0,
                        "care_site_id": row.care_site_id if row.care_site_id is not None else 0,
                        "person_source_value": row.person_source_value,
                        "gender_source_value": row.gender_source_value,
                        "gender_source_concept_id": row.gender_source_concept_id if row.gender_source_concept_id is not None else 0,
                        "race_source_value": row.race_source_value,
                        "race_source_concept_id": row.race_source_concept_id if row.race_source_concept_id is not None else 0,
                        "ethnicity_source_value": row.ethnicity_source_value,
                        "ethnicity_source_concept_id": row.ethnicity_source_concept_id if row.ethnicity_source_concept_id is not None else 0,
                    }
...
async def create(dataset_id: int, body: dict):
    ...
    async for session in get_db_session():
        try:
            ...
            required_fields = ['gender_concept_id', 'race_concept_id', 'ethnicity_concept_id']
            ...
            result = await session.execute(check_dataset_sql, {"dataset_id": dataset_id})
            ...
            if body.get('birth_datetime'):
                try:
                    birth_datetime = datetime.fromisoformat(body['birth_datetime'].replace('Z', '+00:00'))
                except ValueError:
                    raise ProblemException(
                        status=400,
                        title="Bad Request",
...
async def put(dataset_id: int, id: int, body: dict):
    ...
    async for session in get_db_session():
        try:
            # Validate required fields
            required_fields = ['person_id', 'gender_concept_id', 'race_concept_id', 'ethnicity_concept_id']
            for field in required_fields:
                if field not in body:
                    raise ProblemException(
                        status=400,
            ...
            result = await session.execute(check_person_sql, {
                "dataset_id": dataset_id,
                "person_id": int(id)
            })
            if result.fetchone() is None:
...
async def delete(dataset_id: str, id: str):
    ...
    async for session in get_db_session():
        try:
            ...
            await session.execute(delete_dataset_sql, {
                "dataset_id": dataset_id,
                "person_id": int(id)
            })
            
            # Delete from person table
            await session.execute(delete_person_sql, {
                "person_id": int(id)
            })
...
async def patch_user(dataset_id: str, id: str, body: dict):
    ...
    async for session in get_db_session():
        try:
            ...
            person_params = {
                "person_id": int(id),
                "gender_concept_id": body.get('gender_concept_id', current_person.gender_concept_id),
                "year_of_birth": body.get('year_of_birth', current_person.year_of_birth),
                "month_of_birth": body.get('month_of_birth', current_person.month_of_birth),
                "day_of_birth": body.get('day_of_birth', current_person.day_of_birth),
                "birth_datetime": birth_datetime if 'birth_datetime' in body else current_person.birth_datetime,
                "race_concept_id": body.get('race_concept_id', current_person.race_concept_id),
                "ethnicity_concept_id": body.get('ethnicity_concept_id', current_person.ethnicity_concept_id),
                ...
                "person_source_value": body.get('person_source_value', current_person.person_source_value),
                "gender_source_value": body.get('gender_source_value', current_person.gender_source_value),
                "gender_source_concept_id": body.get('gender_source_concept_id', current_person.gender_source_concept_id),
                "race_source_value": body.get('race_source_value', current_person.race_source_value),
                "race_source_concept_id": body.get('race_source_concept_id', current_person.race_source_concept_id),
                "ethnicity_source_value": body.get('ethnicity_source_value', current_person.ethnicity_source_value),
                "ethnicity_source_concept_id": body.get('ethnicity_source_concept_id', current_person.ethnicity_source_concept_id)
            }
